...
def calculate_absolute_devation(od_candidate_x, od_candidate_y, depot_data, current_od):
...
def calculate_value_of_deviation(deviation, current_od):
    return deviation * current_od['time_per_distance'] * current_od['value_of_time']


def calculate_incentive(deviation, fixed_incentive_arg, variable_incentive_arg):
    return fixed_incentive_arg + (deviation * variable_incentive_arg)
...
def evaluate_od_candidates(candidates_data, current_od_data, depot_data, fixed_incentive, variable_incentive):
    ...
    df_candidates['Absolute_Deviation'] = df_candidates.apply(
        lambda row: calculate_absolute_devation(row['x'], row['y'], depot_data, current_od_data), axis=1)

    df_candidates['Value_of_deviation_time'] = df_candidates.apply(
        lambda row: calculate_value_of_deviation(row['Absolute_Deviation'], current_od_data), axis=1)

    df_candidates['Incentive_Offered'] = df_candidates.apply(
        lambda row: calculate_incentive(row['Absolute_Deviation'], fixed_incentive, variable_incentive), axis=1)
    ...
    trip_evaluation_dict = {"OD_index": current_od_data['_index'], "Selected_Customer_Index": int(df_candidates.iloc[0]['_index']), "Incentive_Offered": df_candidates.iloc[0]['Incentive_Offered'], "Perceived_Gain": df_candidates.iloc[0]['Perceived_Gain']}
...
def main():
    ...
    od_test = {'y_home': 14.78621164329888, '_index': 9, 'value_of_time': 5.503391576577617, '_text7': 19, '_text4': 13,
               '_text5': 16, '_text2': 7, 'income_level': 2.0, 'arrival_rate': 20.0, '_text3': 10, '_text1': 4,
    ...
    variable_incentive_test = 2.0

    evaluate_od_candidates(candidates_test, od_test, depot_test, fixed_incentive_test, variable_incentive_test)
